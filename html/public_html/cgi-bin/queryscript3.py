...
sql="SELECT DISTINCT Genes.Gene_ID, Gene_Title, Strain, Isolate, Expression_value FROM Expression INNER JOIN Isolates ON Expression.Sample_ID=Isolates.Sample_ID INNER JOIN Strains ON Isolates.Sample_ID=Expression.Sample_ID INNER JOIN Probes ON Probes.ID_REF=Expression.ID_REF INNER JOIN Genes ON Genes.Gene_ID=Probes.Gene_ID WHERE Genes.Gene_ID=%s AND Isolate='burn wound' AND Expression_value>13 ORDER BY Expression_value DESC LIMIT 30" #query
...
if form.has_key('title'):
    ...
    print 'Form does not have key: title'
...
cursor=db.cursor()
cursor.execute(exprsqlbw,form['title'].value)
...
cursor.execute(mtmsql,form['title'].value)
...
cursor.execute(pmsql,form['title'].value)
...
cursor.execute(ivbcsql,form['title'].value)
...
cursor.execute(ivpcsql,form['title'].value)
